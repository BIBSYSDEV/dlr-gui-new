...
const FeedSideBar = styled.div`
  padding-top: 1.5rem;
  padding-left: 1rem;
  background-color: ${Colors.DLRYellow2};
  width: 17rem;
  display: flex;
  align-items: left;
  flex-direction: column;
    ...
    <div>
      {width >= DeviceWidths.lg ? (
        <>
          ...
          <FeedSideBar>
            <Typography variant="h3">{t('dashboard.feeds')}</Typography>
            <Button
              color="primary"
              variant="outlined"
            ...
            <Button
              ...
              variant="outlined"
              data-testid="feed-button"
              onClick={() => generateFeedUrl(queryObject, 'atom_1.0')}
              startIcon={<RssFeedIcon />}>
              <Typography variant="button">{t('feeds.atom')}</Typography>
            </Button>
          </FeedSideBar>
        </>
      ) : (
